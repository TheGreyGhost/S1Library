package com.shieldbug1.lib.repackage.speedytools.common.utilities;
 ...
 * 
 * 
 * Note: This is taken straight from <a href="TGG's github">https://github.com/TheGreyGhost/SpeedyTools/blob/master/src/main/java/speedytools/common/utilities/OpenGLdebugging.java</a>
 * This version has been slightly modified by me.
 */
public class OpenGLdebugging
{
	public static class GLproperty
	{
		...
		public GLproperty(int constant, String name, String description, String category, String fetchCommand)
		{
	...
	public static final GLproperty[] PROPERTY_LIST =
		{
	...
	public static void dumpAllIsEnabled()
	{
		for (int i = 0; i < PROPERTY_LIST.length; ++i) {
			if (PROPERTY_LIST[i].fetchCommand.equals("glIsEnabled()")) {
				System.out.print(PROPERTY_LIST[i].name + ":");
				System.out.print(GL11.glIsEnabled(PROPERTY_LIST[i].gLconstant));
				System.out.println(" (" + PROPERTY_LIST[i].description + ")");
			}
		}
	}
	
	public void dumpAllType(String type)
	{
		for (int i = 0; i < PROPERTY_LIST.length; ++i)
		{
			if (PROPERTY_LIST[i].category.equals(type))
			{
				System.out.print(PROPERTY_LIST[i].name + ":");
				System.out.println(this.getPropertyAsString(i));
				System.out.println(" (" + PROPERTY_LIST[i].description + ")");
			}
		}
	}

	private String getPropertyAsString(int propertyListIndex)
	{
		int gLconstant = PROPERTY_LIST[propertyListIndex].gLconstant;
		if (PROPERTY_LIST[propertyListIndex].fetchCommand.equals("glIsEnabled()"))
		{
			return "" + GL11.glIsEnabled(gLconstant);
		}

		if (PROPERTY_LIST[propertyListIndex].fetchCommand.equals("glGetBooleanv()"))
		{
